...
# ==========================================================
# Camera center ROI
# Only the middle 50% of the image is visible to vision.
# For 640 px width:
# left blocked  : 0   ~ 159
# active region : 160 ~ 479
# right blocked : 480 ~ 639
# ==========================================================
...
BOTTLE_MIN_HEIGHT = 260.0
BOTTLE_MAX_HEIGHT = 292.0
BOTTLE_MIN_BOTTOM_Y = 320.0
...
# 分类方向：RoboMaster 底盘 y>0 左移，y<0 右移。
TENNIS_SORT_Y_SIGN = 1.0
BOTTLE_SORT_Y_SIGN = -1.0
SORT_LATERAL_STEP_M = 0.08
SORT_LATERAL_MAX_STEPS = 20
SORT_TAPE_CLEAR_HITS = 3
SORT_TAPE_CHECK_INTERVAL_S = 0.12
...
def apply_center_roi_mask(frame_bgr):
    """
    Hide the left 25% and right 25% of the camera image.

    The output keeps the original image size so all existing
    YOLO coordinates and bottom_x control logic remain unchanged.

    Hidden regions are filled with white so they cannot trigger
    the black-marker detector.
    """
    ...
    left_x = int(round(w * ROI_LEFT_RATIO))
    right_x = int(round(w * ROI_RIGHT_RATIO))

    # White is intentionally used here.
    # If black were used, the black-marker detector would see
    # the blocked regions as giant black objects.
    masked[:, :left_x] = 255
    masked[:, right_x:] = 255
...
def detect_one_frame(
    ...
    # and YOLO.
    # ------------------------------------------------------
    vision_frame = apply_center_roi_mask(
        frame_bgr
    )
...
class AsyncVision:
    ...
    def black_marker_visible(self):
        frame, _ = self._get_frame()

        if frame is None:
            return None

        vision_frame = apply_center_roi_mask(frame)
        marker_found, _, _ = detect_black_rectangle(vision_frame)
        return bool(marker_found)
        ...
        try:
            ...
            while not self.stop_event.is_set():
                ...
                if frame is not None:
                    ...
                    # Show exactly what the vision system can see.
                    vision_frame = apply_center_roi_mask(
                        frame
                    )
                    ...
                    if marker_found and marker_box is not None:
...
def sort_lateral_direction(class_name):
    if class_name == "tennis_ball":
        return TENNIS_SORT_Y_SIGN, "LEFT / 网球区"

    return BOTTLE_SORT_Y_SIGN, "RIGHT / 水瓶区"


def strafe_until_tape_clear(
    chassis,
    vision,
    class_name,
):
    y_sign, zone_name = sort_lateral_direction(class_name)
    clear_hits = 0

    for step in range(1, SORT_LATERAL_MAX_STEPS + 1):
        if vision.is_stopped():
            raise KeyboardInterrupt

        marker_visible = vision.black_marker_visible()

        if marker_visible is None:
            time.sleep(SORT_TAPE_CHECK_INTERVAL_S)
            continue
        ...
        if not marker_visible:
            ...
            print(
                f"[SORT] 黑胶带未出现 "
                f"({clear_hits}/{SORT_TAPE_CLEAR_HITS})"
            )

            if clear_hits >= SORT_TAPE_CLEAR_HITS:
                print(f"[SORT] 已离开黑胶带区域 -> {zone_name}")
                return
        ...
        clear_hits = 0
        direction = "左" if y_sign > 0 else "右"
        vision.set_status(f"SORT / STRAFE {direction}")

        print(
            f"[SORT] 仍识别到黑胶带，第 {step} 次向{direction}平移 "
            f"{SORT_LATERAL_STEP_M * 100:.0f}cm"
        )

        chassis.move(
            x=0,
            y=float(y_sign * SORT_LATERAL_STEP_M),
            z=0,
    ...
    raise RuntimeError("达到最大平移距离后仍能识别到黑胶带，已停止分类动作")


def prepare_next_search_without_scan(
    arm,
    vision,
):
    vision.set_force_yolo(False)
    vision.set_status("RETURN SEARCH / NO TURN SCAN")
    move_arm_to_observe_pose(arm)
    vision.set_target("auto")
    vision.set_profile(None)

    print("[RETURN_SEARCH] 已删除转头扫描，保持当前朝向。")
...
def place_to_sort_side(
    ...
    抓取成功后的分类动作：
        ...
        后退回原位置
    """
    _, zone_name = sort_lateral_direction(class_name)
    ...
    # 1. 抓起物体后先后退，并保持夹持。
    vision.set_status("SORT / BACK UP")
    print(f"[SORT] 抓取成功后先后退 {POST_GRASP_BACK_M * 100:.0f}cm")
    chassis.move(
        x=-POST_GRASP_BACK_M,
        y=0,
        z=0,
    ...
    # 2. 后退完成后，机械臂回正并保持夹持。
    vision.set_status("SORT / RECENTER ARM")
    move_arm_to_observe_pose(arm)

    # 3. 保持车头方向，只做左右平移，直到中央视野没有黑胶带。
    strafe_until_tape_clear(
        chassis=chassis,
        vision=vision,
        class_name=class_name,
    )

    # 4. 向分类区前进一点，避免物体落在机器人正旁边。
    chassis.move(
        x=DROP_FORWARD_M,
        y=0,
        z=0,
    ...
    vision.set_target("auto")
    vision.set_profile(None)
    vision.set_status("SORT DONE / NO TURN SCAN")

    print("[SORT] 分类放置动作完成，全程未执行分类转向。")
...
def main():
    ...
    try:
        ...
        while sorted_count < args.max_items:
            ...
            # 放置后恢复观察姿态，保持车头方向，不执行转头扫描。
            prepare_next_search_without_scan(
                arm=arm,
                vision=vision,
            )
