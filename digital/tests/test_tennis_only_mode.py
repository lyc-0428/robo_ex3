...
class TennisOnlyModeTests(unittest.TestCase):
    ...
    def test_large_markers_cover_every_scheduled_chassis_distance(self):
        ...
        for name, sign in (("tennis_zone_marker", 1.0), ("bottle_zone_marker", -1.0)):
            ...
            self.assertEqual((size_x, size_y), (1.40, 0.90))
            self.assertAlmostEqual(pose_y, sign * 0.60)
            lower = abs(pose_y) - size_y * 0.5
            upper = abs(pose_y) + size_y * 0.5
            self.assertLessEqual(lower, 0.25)
            self.assertGreaterEqual(upper, 0.85)
    ...
    def test_all_cycles_share_one_fixed_world_home_and_reject_zero_approach(self):
        action = (DIGITAL / "actions/grasp_bottle_tennis.py").read_text(
            encoding="utf-8"
        )
        self.assertIn("self._sorting_home_pose = self._identity.pose2d()", action)
    ...
    def test_nominal_classification_turns_are_opposite_in_place_pivots(self):
        action = (DIGITAL / "actions/grasp_bottle_tennis.py").read_text(
            encoding="utf-8"
        )
        self.assertIn(
