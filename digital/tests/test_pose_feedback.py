...
class PoseFeedbackTests(unittest.TestCase):
    ...
    def test_bottle_and_tennis_ninety_degree_pivots_are_opposite(self):
        controller = PoseController()
        current = Pose2D(0.0, 0.0, 0.0)
        ...
        for output in (tennis, bottle):
            self.assertAlmostEqual(output.wheels[0], -output.wheels[1])
            self.assertAlmostEqual(output.wheels[2], -output.wheels[3])
