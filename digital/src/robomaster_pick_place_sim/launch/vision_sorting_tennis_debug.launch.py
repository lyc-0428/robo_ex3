"""Frozen six-tennis-ball debug profile, independent of nominal tuning."""
...
def generate_launch_description():
    ...
    static_launch = str(
        Path(package_share) / "launch" / "static_model_tennis_debug.launch.py"
    )
    params_file = str(
        Path(package_share) / "config" / "vision_sorting_tennis_debug.yaml"
    )
    ...
    actions = PathJoinSubstitution([digital_root, "actions", "tennis_debug"])
    models = PathJoinSubstitution([digital_root, "models", "tennis_debug"])
    logs = PathJoinSubstitution([digital_root, "logs", "tennis_debug"])
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument(
            "scenario",
            default_value="tennis_only",
            description="Frozen tennis-only placement debugging scenario",
        ),
        DeclareLaunchArgument("bottle_confidence", default_value="0.10"),
