...
cleanup_stale_runtime() {
  ...
  local -a stale_pids=()
  local pattern
  pattern='ros2 launch robomaster_pick_place_sim vision_sorting_improved.launch.py|ign gazebo|ignition-gazebo|robot_state_publisher|ros_gz_bridge.*parameter_bridge|ros_ign_bridge.*parameter_bridge|actions/yolo_detector.py|actions/grasp_bottle_tennis.py|actions/spawn_sorting_scene.py|rqt_image_view'
  mapfile -t candidates < <(pgrep -u "$(id -u)" -f "${pattern}" || true)
  for pid in "${candidates[@]}"; do
    [[ "${pid}" == "$$" || "${pid}" == "${PPID}" ]] && continue
    [[ -r "/proc/${pid}/environ" ]] || continue
    environment="$(tr '\0' '\n' < "/proc/${pid}/environ" 2>/dev/null || true)"
    if grep -Eq '^IGN_PARTITION=team21_lyc(_[0-9]+)?$' <<< "${environment}" \
      || grep -Eq '^GZ_PARTITION=team21_lyc(_[0-9]+)?$' <<< "${environment}"; then
      stale_pids+=("${pid}")
    fi
  done
...
RUN_PARTITION="${ROBO_EX3_PARTITION:-team21_lyc_$$}"
export IGN_PARTITION="${RUN_PARTITION}"
export GZ_PARTITION="${RUN_PARTITION}"

# Gazebo remains visible on the Jetson.  The restored bottle/tennis version
# opens its annotated image locally by default for confidence verification.
# Tennis-only debugging stays lighter; the environment variable always wins.
if [[ -n "${ROBO_EX3_SHOW_IMAGE:-}" ]]; then
  SHOW_IMAGE="${ROBO_EX3_SHOW_IMAGE}"
elif [[ "${SCENARIO}" == "nominal" ]]; then
  SHOW_IMAGE="true"
else
  SHOW_IMAGE="false"
fi
...
setsid ros2 launch robomaster_pick_place_sim vision_sorting_improved.launch.py \
