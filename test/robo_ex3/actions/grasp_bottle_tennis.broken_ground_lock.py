...
class GraspBottleTennisAction(GraspCubeAction):
    ...
    def _set_tennis_ground_lock(self, locked):
        """Attach a tennis ball to the ground, or release it after grasping."""
        object_id = (
            self._unlocked_tennis_id if locked else self._locked_object_id
        )
        if object_id is None:
            if locked:
                return
            raise GraspActionError("tennis ground unlock requested without a target ID")
        if not object_id.startswith("task_object_"):
            raise GraspActionError(f"invalid tennis object ID: {object_id}")
        if not locked and self._unlocked_tennis_id == object_id:
            return

        operation = "attach" if locked else "detach"
        topic = f"/sorting/{object_id}/ground_lock/{operation}"
        self._run(
            [
                "ign",
                "topic",
        ...
        self._wait_sim_duration(0.05)
        if locked:
            self._unlocked_tennis_id = None
            self._publish_status(
                f"TENNIS GROUND LOCKED | id={object_id}"
            )
        else:
            self._unlocked_tennis_id = object_id
            self._publish_status(
                f"TENNIS GROUND UNLOCKED AFTER GRIP CLOSED | id={object_id}"
            )

    def _move_gripper(self, label, start, target):
        # Pin the ball before opening the fingers.  On pickup, keep it pinned
        # throughout finger closure and release it only once the grip is shut.
        if (
            self._use_tennis_grasp
            and self._unlocked_tennis_id is not None
            and label in ("释放物品", "安全张开夹爪")
        ):
            self._set_tennis_ground_lock(True)
        super()._move_gripper(label, start, target)
        if self._use_tennis_grasp and label == "缓慢闭合夹爪":
            self._set_tennis_ground_lock(False)

    def _move_chassis_final_insert(self, held_positions, forward=True):
        # An empty grasp unlocks the ball after finger closure too.  Re-pin it
        # before the chassis backs away; a real grasp leaves a visible gap and
        # therefore does not satisfy _is_fully_closed.
        if (
            not forward
            and self._use_tennis_grasp
            and self._unlocked_tennis_id is not None
            ...
            index = {name: i for i, name in enumerate(POSITION_JOINTS)}
            if self._is_fully_closed(held_positions, index):
                self._set_tennis_ground_lock(True)
        super()._move_chassis_final_insert(held_positions, forward=forward)
    ...
    def _safe_home_pose(self, initial, current, index):
        ...
        if self._unlocked_tennis_id is not None:
            self._set_tennis_ground_lock(True)
