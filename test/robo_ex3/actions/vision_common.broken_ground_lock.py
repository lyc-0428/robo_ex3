...
def make_slot_layout(seed, radius, angles_degrees):
    ...
    classes = [BOTTLE, BOTTLE, TENNIS, TENNIS]
    layout = []
...
<sdf version="1.7">
  <model name="{name}">
    ...
    <plugin filename="ignition-gazebo-detachable-joint-system"
      ...
      <parent_link>object_link</parent_link>
      <child_model>ground</child_model>
      <child_link>ground_link</child_link>
      <detach_topic>/sorting/{name}/ground_lock/detach</detach_topic>
      <attach_topic>/sorting/{name}/ground_lock/attach</attach_topic>
      <output_topic>/sorting/{name}/ground_lock/state</output_topic>
    </plugin>
  </model>
</sdf>
"""
