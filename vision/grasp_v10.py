        ...
        while step < args.observe_max_steps:
            ...
            if seq == last_det_seq:
                lost_limit = (
                    ...
                    if near_view_lowered and locked_class == "tennis_ball"
                    else OBSERVE_LOST_TIMEOUT_S
                )

                if time.monotonic() - last_seen_time > lost_limit:
                    print(
                        f"[APPROACH] 连续超过{lost_limit:.1f}秒"
                        "没有新的有效目标，停止并恢复。"
                    )
            ...
            if candidate is None:
                ...
                if time.monotonic() - last_seen_time > lost_limit:
                    print(
                        f"[APPROACH] 锁定目标丢失超过{lost_limit:.1f}秒，"
                        "停止并恢复。"
                    )
                    stop_chassis_smooth(chassis, current_x, current_z)
                    return None, None, None
            ...
            if (
                ...
                # --------------------------------------------------
                # 这是本版核心修复：
                # 不再等到最终 PREGRASP 才第一次下降机械臂。
                # 在球刚进入近距离区域时就先下降一小段，
                # 姿态改变后重新识别，然后继续慢速靠近。
                if (
                    locked_class == "tennis_ball"
                    and not near_view_lowered
                ):
                    vision.set_status("NEAR_VIEW / LOWER ARM")
                    ...
                    # 相机姿态改变后，原来的像素位置不能继续用于跟踪。
                    previous_det = None

                    # 丢弃机械臂运动期间积累的旧YOLO序号，
                    # 等下一次真正的新结果再继续控制。
                    last_det_seq = vision.current_det_seq()
                    last_seen_time = time.monotonic()

                    print(
                        "[NEAR_VIEW] 机械臂已进入中间观察姿态，"
                        "重新寻找网球后继续 ALIGN/APPROACH。"
                    )
            ...
            if align_mode:
                ...
                # 若还没进入网球中间观察姿态，明显变远可退回远距APPROACH。
                # 一旦网球已经下降到中间观察姿态，就不再切回高速远距模式，
                # 防止机械臂已下降后底盘再次高速前冲。
                if (
                    size_ratio_to_switch < 0.72
                    and not (
                        locked_class == "tennis_ball"
                        and near_view_lowered
                    )
                ):
                ...
                if size_value < switch_threshold:
                    ...
                    # 网球中间观察姿态下继续保持低速，不切回远距速度。
                    align_hits = 0

                    creep_speed = PREGRASP_CREEP_MPS
