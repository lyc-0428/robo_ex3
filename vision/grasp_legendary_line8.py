...
"""RoboMaster EP 实验三：八点直线固定槽位分类抓取。

保留 v9 已验证的 YOLO、异步视频、学习抓取姿态和夹爪状态判断；
任务层改为沿八个固定点横向平移。当前槽位以画面中心 ROI 识别，底盘
在搜索阶段不前进、不追框、不改变朝向。网球送左侧，水瓶送右侧。

主窗口显示 YOLO，第二个窗口显示黑色防静电胶带的二值识别结果。
底部两侧夹爪区域会在 YOLO 输入和胶带检测中屏蔽，但保留中央抓取通道。
"""
...
# ==========================================================
LINE8_SLOT_COUNT = 8
# “30 mm 标记 + 80 mm 边缘间隔”对应 110 mm 中心距。
LINE8_SLOT_PITCH_M = 0.110
LINE8_LATERAL_SPEED = 0.14
LINE8_START_SLOT = 1
LINE8_SLOT_SETTLE_S = 0.45
LINE8_SLOT_TIMEOUT_S = 2.4
LINE8_SLOT_CENTER_X = IMG_W / 2.0
# Keep original pixel coordinates and bottle height calibration intact.
FRONT_ROI_LEFT = 220
FRONT_ROI_RIGHT = 420
LINE8_SLOT_X_TOL_PX = 24.0
LINE8_FIXED_READY_X_TOL_PX = 24.0
LINE8_FIXED_SIZE_TOL_RATIO = 0.16
LINE8_FIXED_Y_TOL_PX = 32.0

# 分类区：转向后分段前进。至少走过最小距离，且连续多帧看不到胶带才
# 允许停止；达到最大距离则强制停止，避免视觉失败时无限前进。
ZONE_STEP_M = 0.025
ZONE_MIN_TRAVEL_M = 0.125
ZONE_MAX_TRAVEL_M = 0.300
ZONE_CLEAR_FRAMES = 6
ZONE_STEP_SETTLE_S = 0.08

# 黑胶带二值检测参数。只分析地面中下部，并滤掉小黑色噪点。
TAPE_GRAY_THRESHOLD = 72
TAPE_ROI_TOP = 150
TAPE_ROI_BOTTOM = 335
TAPE_ROI_LEFT = 70
TAPE_ROI_RIGHT = 540
TAPE_MIN_AREA_PX = 700.0
TAPE_MAX_AREA_PX = 6500.0
TAPE_MIN_SIDE_PX = 14
TAPE_MAX_SIDE_PX = 130

# 夹爪屏蔽区为底部左右两个多边形，中央通道留给瓶子和网球。
GRIPPER_MASK_LEFT = np.array(
    [[0, IMG_H], [0, 258], [178, 244], [278, IMG_H]],
    dtype=np.int32,
)
GRIPPER_MASK_RIGHT = np.array(
    [[IMG_W, IMG_H], [IMG_W, 258], [462, 244], [362, IMG_H]],
    dtype=np.int32,
)

# 90 degrees at 25 deg/s already takes 3.6 seconds before settling.
CHASSIS_ACTION_TIMEOUT_S = 8.0
LINE8_LOG_DIR = os.environ.get("LINE8_LOG_DIR", os.path.expanduser("~/Desktop/LINE8_logs"))


def point_in_gripper_mask(x, y):
    point = (float(x), float(y))
    return (
        cv2.pointPolygonTest(GRIPPER_MASK_LEFT, point, False) >= 0
        or cv2.pointPolygonTest(GRIPPER_MASK_RIGHT, point, False) >= 0
    )


def apply_gripper_mask(frame, value=114):
    masked = frame.copy()
    masked[:, :FRONT_ROI_LEFT] = value
    masked[:, FRONT_ROI_RIGHT:] = value
    cv2.fillPoly(masked, [GRIPPER_MASK_LEFT, GRIPPER_MASK_RIGHT], value)
    return masked


def build_tape_binary(frame_bgr):
    """Return binary tape mask, accepted contours and an area score."""
    gray = cv2.cvtColor(frame_bgr, cv2.COLOR_BGR2GRAY)
    gray = cv2.GaussianBlur(gray, (5, 5), 0)
    binary = cv2.inRange(gray, 0, TAPE_GRAY_THRESHOLD)

    roi_mask = np.zeros_like(binary)
    roi_mask[
        TAPE_ROI_TOP:TAPE_ROI_BOTTOM,
        TAPE_ROI_LEFT:TAPE_ROI_RIGHT,
    ] = 255
    cv2.fillPoly(roi_mask, [GRIPPER_MASK_LEFT, GRIPPER_MASK_RIGHT], 0)
    # Tape-only exclusion: moving jaws occupy this band. Keep YOLO's
    # central grasp channel intact; missing tape here means unknown.
    roi_mask[230:, :] = 0
    roi_mask[:, :FRONT_ROI_LEFT] = 0
    roi_mask[:, FRONT_ROI_RIGHT:] = 0
    ...
    for contour in contours:
        ...
        if not (TAPE_MIN_AREA_PX <= area <= TAPE_MAX_AREA_PX):
            continue
        if min(w, h) < TAPE_MIN_SIDE_PX or max(w, h) > TAPE_MAX_SIDE_PX:
            continue
        if area / float(w * h) < 0.50:
            continue
        hull_area = float(cv2.contourArea(cv2.convexHull(contour)))
        if hull_area <= 0.0 or area / hull_area < 0.55:
            continue
        accepted.append((contour, (x, y, w, h), area))
        cv2.drawContours(clean, [contour], -1, 255, -1)

    score = sum(item[2] for item in accepted)
    return clean, accepted, score
...
def detect_one_frame(model, frame_bgr, target, use_cuda):
    ...
    inference_frame = apply_gripper_mask(frame_bgr, value=(114, 114, 114))
    ...
    for box in result.boxes:
        ...
        # A box cut by the ROI edge has an unreliable size for approach.
        if x1 <= FRONT_ROI_LEFT + 2 or x2 >= FRONT_ROI_RIGHT - 2:
            continue
        ...
        # 检测框落点位于夹爪本体时，直接丢弃这个候选。
        if point_in_gripper_mask(bottom_x, bottom_y):
            continue
...
class AsyncVision:
    def __init__(
        ...
        self.latest_binary = np.zeros((IMG_H, IMG_W), dtype=np.uint8)
        self.latest_tape_boxes = []
    ...
    def _capture_loop(self):
        while not self.stop_event.is_set():
            ...
            binary, accepted, score = build_tape_binary(frame)
            boxes = [item[1] for item in accepted]
            with self.line_lock:
                self.latest_binary = binary
                self.latest_tape_boxes = boxes
                self.line_score = float(score)
                self.line_present = bool(boxes)
                self.line_seq += 1
    ...
    def line_snapshot(self):
        with self.line_lock:
            return (
                self.line_seq,
                bool(self.line_present),
                float(self.line_score),
                list(self.latest_tape_boxes),
                self.latest_binary.copy(),
            )

    def _display_loop(self):
        period = 1.0 / self.display_fps

        window_name = "EXP3 Legendary Line8"
        binary_window = "LINE8 Tape Binary"
        ...
        try:
            ...
            cv2.resizeWindow(binary_window, 640, 360)
            ...
            while not self.stop_event.is_set():
                ...
                if frame is not None:
                    ...
                    _, line_present, line_score, tape_boxes, binary = (
                        self.line_snapshot()
                    )
                    ...
                    cv2.fillPoly(
                        overlay,
                        [GRIPPER_MASK_LEFT, GRIPPER_MASK_RIGHT],
                        (180, 0, 180),
                    )
                    shown = cv2.addWeighted(overlay, 0.28, shown, 0.72, 0)
                    shown[:, :FRONT_ROI_LEFT] = shown[:, :FRONT_ROI_LEFT] // 4
                    shown[:, FRONT_ROI_RIGHT:] = shown[:, FRONT_ROI_RIGHT:] // 4
                    cv2.rectangle(shown, (FRONT_ROI_LEFT, 33),
                                  (FRONT_ROI_RIGHT - 1, h - 1), (0, 255, 255), 1)
                    for tx, ty, tw, th in tape_boxes:
                        cv2.rectangle(
                            shown,
                            (tx, ty),
                            (tx + tw, ty + th),
                    ...
                    line = (
                        ...
                        f"TAPE={'YES' if line_present else 'NO'} "
                        f"score={line_score:.0f} | "
                    ...
                    binary_shown = cv2.cvtColor(binary, cv2.COLOR_GRAY2BGR)
                    cv2.putText(
                        binary_shown,
                        (
                            f"TAPE {'CANDIDATE' if line_present else 'NOT VISIBLE'} "
                            f"score={line_score:.0f}"
                        ),
                        (8, 24),
                        cv2.FONT_HERSHEY_SIMPLEX,
                    ...
                    cv2.imshow(binary_window, binary_shown)
            ...
            try:
                ...
                pass
            try:
                cv2.destroyWindow(binary_window)
            except Exception:
                pass
...
def _wait_arm_action(action, label):
    ...
    raise RuntimeError(f"ARM timeout: {label}; stop task before further motion")
...
def configure_gripper(class_name):
    exp2.GRIP_POWER = 20 if class_name == "bottle" else 30
    ...
    if class_name == "bottle":
        print(
            "bottle gripper: power=20, closed_fast=2.0s"
        )
...
def _wait_chassis_action(action, chassis, label):
    try:
        ok = action.wait_for_completed(timeout=CHASSIS_ACTION_TIMEOUT_S)
    except TypeError:
        ok = action.wait_for_completed(CHASSIS_ACTION_TIMEOUT_S)
    if ok:
        return True
    print(f"[CHASSIS] {label} timeout, force stop")
    try:
...
def locate_slot_target(
    vision,
    timeout=LINE8_SLOT_TIMEOUT_S,
    center_x=LINE8_SLOT_CENTER_X,
    x_tolerance=LINE8_SLOT_X_TOL_PX,
    required_hits=STABLE_HITS,
):
    """Identify only the object occupying the currently aligned slot."""
    ...
    while time.monotonic() < deadline:
        ...
        candidates = [
            d for d in detections
            if abs(float(d["bottom_x"]) - float(center_x)) <= x_tolerance
        ]
        candidate = None
        if candidates:
            candidate = min(
                candidates,
                key=lambda d: (
                    abs(float(d["bottom_x"]) - float(center_x)),
                    -float(d["bottom_y"]),
                    -float(d["conf"]),
                ),
            )
        ...
        if len(valid) < required_hits:
            continue
        counts = Counter(item["class"] for item in valid)
        class_name, hits = counts.most_common(1)[0]
        if hits < required_hits:
            continue
        same = [item for item in valid if item["class"] == class_name]
        return {
            "class": class_name,
            "conf": float(np.median([item["conf"] for item in same])),
            "bottom_x": float(np.median([item["bottom_x"] for item in same])),
            "bottom_y": float(np.median([item["bottom_y"] for item in same])),
            "width": float(np.median([item["width"] for item in same])),
            "height": float(np.median([item["height"] for item in same])),
        }
    return None


def fixed_slot_ready(vision, profile, timeout=2.2):
    """Verify the learned grasp view without sending any chassis command."""
    class_name = profile["target"]
    deadline = time.monotonic() + float(timeout)
    good = deque(maxlen=5)
    last_seq = vision.current_det_seq()
    last_loc = None
    ...
    while time.monotonic() < deadline:
        if vision.is_stopped():
            raise KeyboardInterrupt
        seq, _, _, _ = vision.snapshot()
        if seq <= last_seq:
            time.sleep(0.01)
            continue
        last_seq = seq
        loc = get_latest_target_detection(
            vision, class_name, ref_x=float(profile["bottom_x_px"])
        )
        if loc is None:
            good.append(False)
            continue
        last_loc = loc
        x_ok = abs(loc["bottom_x"] - profile["bottom_x_px"]) <= (
            LINE8_FIXED_READY_X_TOL_PX
        )
        if class_name == "bottle":
            ratio = loc["height"] / float(profile["bbox_height_px"])
        else:
            ratio = loc["width"] / float(profile["bbox_width_px"])
        size_ok = abs(ratio - 1.0) <= LINE8_FIXED_SIZE_TOL_RATIO
        y_ok = abs(loc["bottom_y"] - profile["bottom_y_px"]) <= (
            LINE8_FIXED_Y_TOL_PX
        )
        print(
            f"[GRASP CHECK] class={class_name} "
            f"x={loc['bottom_x']:.1f} expected={profile['bottom_x_px']:.1f} "
            f"tol={LINE8_FIXED_READY_X_TOL_PX} pass={x_ok}; "
            f"size_ratio={ratio:.3f} tol={LINE8_FIXED_SIZE_TOL_RATIO} pass={size_ok}; "
            f"y={loc['bottom_y']:.1f} expected={profile['bottom_y_px']:.1f} "
            f"tol={LINE8_FIXED_Y_TOL_PX} pass={y_ok}"
        )
        good.append(bool(x_ok and size_ok and y_ok))
        if len(good) == good.maxlen and sum(good) >= 3:
            print(
                f"[FIXED READY] {class_name} x={loc['bottom_x']:.1f} "
                f"y={loc['bottom_y']:.1f} ratio={ratio:.3f}"
            )
            return loc

    if last_loc is not None:
        print(
            f"[FIXED NOT READY] {class_name} x={last_loc['bottom_x']:.1f} "
            f"y={last_loc['bottom_y']:.1f}"
        )
    else:
        print(f"[FIXED NOT READY] {class_name} not visible in grasp pose")
    return None


def approach_decision(loc, profile):
    """Never advance when close enough, too large, or off the locked lane."""
    ratio = loc["height"] / profile["bbox_height_px"] if profile["target"] == "bottle" else loc["width"] / profile["bbox_width_px"]
    dx = loc["bottom_x"] - profile["bottom_x_px"]
    dy = loc["bottom_y"] - profile["bottom_y_px"]
    if abs(dx) > LINE8_FIXED_READY_X_TOL_PX:
        return "off_center", 0.0
    if ratio > 1.12 or dy > 32:
        return "too_close", 0.0
    if 0.92 <= ratio <= 1.12 and abs(dy) <= 32:
        return "ready", 0.0
    # Conflicting depth cues must not make the robot push forward.
    if ratio >= 0.92 or dy >= -12:
        return "depth_conflict", 0.0
    return "advance", 0.020 if ratio < 0.80 else 0.010


def approach_slot(chassis, vision, profile, log_path, slot):
    distance = 0.0
    ...
    while time.monotonic() - started < 60:
        if vision.is_stopped():
            raise KeyboardInterrupt
        seq, stamp, _, detections = vision.snapshot()
        now = time.monotonic()
        if seq == last_seq or stamp <= after or now - stamp > MAX_DET_AGE_S:
            if now - last_seen > 3.0:
                return False, distance, "stale_or_lost_video"
            time.sleep(0.02)
            continue
        last_seq = seq
        candidates = [d for d in detections if d["class"] == profile["target"]
                      and abs(d["bottom_x"] - profile["bottom_x_px"]) <= 45]
        if previous is not None:
            candidates = [d for d in candidates
                          if abs(d["bottom_x"] - previous["bottom_x"]) < 35
                          and abs(d["bottom_y"] - previous["bottom_y"]) < 60]
        if not candidates:
            ready_hits = 0
            if now - last_seen > 3.0:
                return False, distance, "target_lost"
            continue
        loc = min(candidates, key=lambda d: abs(d["bottom_x"] - profile["bottom_x_px"]))
        previous = loc
        last_seen = now
        decision, step = approach_decision(loc, profile)
        log_line8(log_path, slot=slot, state="APPROACH", decision=decision,
                  forward_m=round(distance, 3), detection=loc)
        vision.set_status(f"SLOT {slot} / {decision} {distance:.3f}m")
        if decision == "ready":
            ready_hits += 1
            if ready_hits >= 3:
                return True, distance, "ready"
            continue
        ready_hits = 0
        if decision != "advance":
            return False, distance, decision
        if distance + step > 0.20 + 1e-6:
            return False, distance, "approach_distance_limit"
        if not _wait_chassis_action(
            chassis.move(x=step, y=0, z=0, xy_speed=0.08), chassis, "grasp approach"
        ):
            raise RuntimeError("Approach timeout: actual position unknown; stop task")
        distance += step
        time.sleep(0.2)
        after = time.monotonic()
        last_seen = after
    return False, distance, "approach_timeout"


def return_to_scan_line(chassis, distance):
    if distance > 0 and not _wait_chassis_action(
        chassis.move(x=-distance, y=0, z=0, xy_speed=0.10), chassis, "return to scan line"
    ):
        raise RuntimeError("Return timeout: stop before moving to another slot")
    time.sleep(0.3)


def move_lateral_slots(chassis, slot_delta, pitch_m, lateral_sign):
    distance = float(slot_delta) * float(pitch_m) * float(lateral_sign)
    if abs(distance) < 1e-6:
        return True
    print(f"[LINE8] lateral move y={distance:+.3f}m")
    action = chassis.move(
        x=0,
        y=distance,
        z=0,
        xy_speed=LINE8_LATERAL_SPEED,
    )
    ok = _wait_chassis_action(action, chassis, "line8 lateral")
    time.sleep(LINE8_SLOT_SETTLE_S)
    return ok


def open_line8_log():
    try:
        os.makedirs(LINE8_LOG_DIR, exist_ok=True)
        stamp = time.strftime("%Y%m%d_%H%M%S")
        return os.path.join(LINE8_LOG_DIR, f"line8_{stamp}.jsonl")
    except Exception as error:
        print("[LOG] cannot create log directory:", repr(error))
        return None


def log_line8(path, **record):
    record = dict(record)
    ...
    try:
        ...
        print("[LOG] write failed:", repr(error))


def place_to_zone_by_tape(
    arm,
    chassis,
    gripper,
    class_name,
    ...
    """Use a measured drop distance; tape visibility is diagnostic only."""
    turn_deg, zone_name = sort_direction(class_name)
    vision.set_status(f"ZONE {zone_name} / TURN")
    if not _wait_chassis_action(
        chassis.move(x=0, y=0, z=turn_deg, z_speed=DROP_Z_SPEED),
        chassis,
        "turn to class zone",
    ):
        return False, 0.0, "turn_timeout"
    time.sleep(0.35)

    travelled = 0.0
    stop_reason = "measured_distance"
    while travelled + 1e-6 < zone_distance_m:
        if vision.is_stopped():
            raise KeyboardInterrupt
        step_m = min(ZONE_STEP_M, zone_distance_m - travelled)
        if not _wait_chassis_action(
            chassis.move(
                x=step_m,
                y=0,
            ...
            return False, travelled, "drive_timeout"
        travelled += step_m
        time.sleep(ZONE_STEP_SETTLE_S)
        _, present, score, _, _ = vision.line_snapshot()
        print(
            f"[ZONE] travel={travelled:.3f}m tape={present} "
            f"score={score:.0f} (diagnostic only)"
        )

    if not _wait_arm_action(arm.move(x=0, y=-DROP_LOWER_MM), "zone lower"):
        return False, travelled, "lower_timeout"
    time.sleep(0.25)
    gripper.open(power=DROP_OPEN_POWER)
    time.sleep(DROP_OPEN_TIME)
    if not _wait_arm_action(arm.move(x=0, y=DROP_LOWER_MM), "zone lift"):
        return False, travelled, "lift_timeout"
    if not _wait_chassis_action(
        chassis.move(x=-travelled, y=0, z=0, xy_speed=DROP_XY_SPEED),
        chassis, "return from class zone",
    ):
        return False, travelled, "return_timeout"
    if not _wait_chassis_action(
        chassis.move(x=0, y=0, z=-turn_deg, z_speed=DROP_Z_SPEED),
        chassis, "restore line heading",
    ):
        return False, travelled, "heading_timeout"
    time.sleep(0.35)

    return True, travelled, stop_reason
...
def run_line8_sort(
    ...
    """Visit eight fixed slots from the configured starting slot."""
    log_path = open_line8_log()
    sorted_count = 0
    slot_states = ["PENDING"] * int(args.slot_count)
    current_slot = int(args.start_slot)

    print("============================================================")
    print("LINE8 FIXED-SLOT SORT")
    print(
        f"slots={args.slot_count}, start={args.start_slot}, "
        f"pitch={args.slot_pitch_m:.3f}m, lateral_sign={args.lateral_sign:+.0f}"
    )
    print("tennis_ball -> LEFT, bottle -> RIGHT")
    print("============================================================")
    ...
    for slot in range(int(args.start_slot), int(args.slot_count) + 1):
        if sorted_count >= args.max_items:
            break
        if slot != current_slot:
            if not move_lateral_slots(
                chassis,
                slot - current_slot,
                args.slot_pitch_m,
                args.lateral_sign,
            ):
                slot_states[slot - 1] = "MOVE_FAILED"
                log_line8(log_path, slot=slot, state="MOVE_FAILED")
                break
            current_slot = slot

        vision.set_status(f"SLOT {slot}/{args.slot_count} / IDENTIFY")
        vision.set_target("auto")
        vision.set_profile(None)
        time.sleep(LINE8_SLOT_SETTLE_S)
        loc = locate_slot_target(vision, timeout=args.slot_timeout)
        _, tape_present, tape_score, _, _ = vision.line_snapshot()

        if loc is None:
            state = "NO_TARGET_UNCONFIRMED"
            slot_states[slot - 1] = state
            log_line8(
                log_path,
                slot=slot,
                state=state,
                tape_present=tape_present,
                tape_score=round(tape_score, 1),
            )
            print(f"[SLOT {slot}] {state}")
            continue

        class_name = loc["class"]
        profile = profiles.get(class_name)
        if profile is None:
            slot_states[slot - 1] = "NO_PROFILE"
            log_line8(log_path, slot=slot, state="NO_PROFILE", cls=class_name)
            continue

        slot_states[slot - 1] = "DETECTED"
        log_line8(
            log_path,
            slot=slot,
            state="DETECTED",
            cls=class_name,
            confidence=round(loc["conf"], 3),
            bottom_x=round(loc["bottom_x"], 1),
            bottom_y=round(loc["bottom_y"], 1),
        )
        if args.nav_only or args.dry_run:
            print(f"[SKIP GRASP] slot={slot}: nav-only/dry-run mode enabled")
            continue

        vision.set_target(class_name)
        vision.set_profile(profile)
        vision.set_status(f"SLOT {slot} / FIXED PREGRASP")
        move_arm_to_profile_pose(arm, profile, "line8 fixed pregrasp")
        ready, forward_m, reason = approach_slot(chassis, vision, profile, log_path, slot)
        if not ready:
            slot_states[slot - 1] = "MISALIGNED"
            log_line8(log_path, slot=slot, state="MISALIGNED", cls=class_name, reason=reason, forward_m=forward_m)
            move_arm_to_observe_pose(arm)
            return_to_scan_line(chassis, forward_m)
            vision.set_target("auto")
            vision.set_profile(None)
            continue
        ...
        if not ok:
            slot_states[slot - 1] = "GRASP_FAILED"
            log_line8(log_path, slot=slot, state="GRASP_FAILED", cls=class_name)
        ...
        return_to_scan_line(chassis, forward_m)
        placed, travelled, stop_reason = place_to_zone_by_tape(
            arm, chassis, gripper, class_name, vision, args.zone_distance_m
        )
        if not placed:
            slot_states[slot - 1] = "PLACE_FAILED"
            log_line8(
                log_path,
                slot=slot,
                state="PLACE_FAILED",
        ...
        log_line8(
            ...
            state="DONE",
            cls=class_name,
            zone="LEFT" if class_name == "tennis_ball" else "RIGHT",
            zone_travel_m=round(travelled, 3),
            zone_stop=stop_reason,
        )
        move_arm_to_observe_pose(arm)
        vision.set_target("auto")
        vision.set_profile(None)
    ...
    vision.set_status(f"LINE8 COMPLETE {sorted_count}/{args.max_items}")
    print("[LINE8] states:", slot_states)
    print("[LINE8] sorted:", sorted_count)
    if log_path:
        print("[LINE8] log:", log_path)
    return 0 if (args.nav_only or sorted_count >= args.max_items) else 7
...
def parse_args():
    ...
    p.add_argument("--zone-distance-m", type=float, default=None,
                   help="Measured drop travel after turning; required for sorting")

    p.add_argument(
        "--slot-count",
        type=int,
        default=LINE8_SLOT_COUNT,
    )

    p.add_argument(
        "--start-slot",
        type=int,
        default=LINE8_START_SLOT,
        help="机器人启动时已对准的槽位编号",
    )
    ...
    p.add_argument(
        "--slot-pitch-m",
        type=float,
        default=LINE8_SLOT_PITCH_M,
        help="相邻黑点中心距，默认0.110m",
    )
    ...
    p.add_argument(
        ...
        choices=[-1.0, 1.0],
        default=-1.0,
        help="从1号到8号横移的y方向；方向反了改为+1",
    )

    p.add_argument(
        "--slot-timeout",
        type=float,
        default=LINE8_SLOT_TIMEOUT_S,
    )

    p.add_argument(
        "--tape-threshold",
        type=int,
        default=TAPE_GRAY_THRESHOLD,
        help="黑胶带灰度阈值，范围1-254",
    )
...
def main():
    global TAPE_GRAY_THRESHOLD
    args = parse_args()
    if args.line8_mode and not (args.nav_only or args.dry_run):
        if args.zone_distance_m is None or not (0 < args.zone_distance_m <= ZONE_MAX_TRAVEL_M):
            print("ERROR: set measured --zone-distance-m (0..0.30m); tape gaps cannot locate a drop zone")
            return 2

    if not (1 <= args.tape_threshold <= 254):
        print("ERROR: tape-threshold 必须在 1..254")
        return 2
    TAPE_GRAY_THRESHOLD = int(args.tape_threshold)

    if args.slot_count < 1 or args.slot_count > 16:
        print("ERROR: slot-count 必须在 1..16")
        return 2
    if args.start_slot < 1 or args.start_slot > args.slot_count:
        print("ERROR: start-slot 超出槽位范围")
        return 2
    if args.slot_pitch_m <= 0.0:
        print("ERROR: slot-pitch-m 必须为正数")
        return 2
    ...
    try:
        ...
        if initialized:
            try:
                ep.chassis.drive_speed(x=0, y=0, z=0, timeout=0.2)
            except Exception:
                pass
